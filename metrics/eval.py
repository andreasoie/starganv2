...
def calculate_evaluation(nets, args, step, mode):
    ...
    if mode == 'reference':
        ...
        loader_ref = get_eval_loader(root=path_ref, img_size=args.img_size, batch_size=args.val_batch_size, drop_last=True)
    path_src = os.path.join(args.val_img_dir, source_domain)
    loader_src = get_eval_loader(root=path_src, img_size=args.img_size, batch_size=args.val_batch_size)

    path_fake = os.path.join(args.eval_dir, f"{mode}", f"step_{step:06d}")
    shutil.rmtree(path_fake, ignore_errors=True)
    os.makedirs(path_fake, exist_ok=True)

    # Load optical images
    for i, x_src in enumerate(tqdm(loader_src, total=len(loader_src), desc=f"Generating", leave=False)):
        N = x_src.size(0)
